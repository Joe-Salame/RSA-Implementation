#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#include<gmp.h>
#include"operations.h"
#include"enc_sign_dec_verify.h"

int main(int argc, char *argv[]){
	if(argc!=2){
		printf("Please input the documents name!\n");
		exit(1);
	}

	FILE *plainData,*keys,*decipher;
	char *e_str=NULL,*N_str=NULL,*d_str=NULL,*p_str=NULL,*q_str=NULL,*Ip_str=NULL;
	ssize_t len=0;
	mpz_t e,d,N,p,q,Ip;

	mpz_inits(e,d,N,p,q,Ip,NULL);

	plainData=fopen(argv[1],"r");
	if(plainData==NULL){
		printf("%s file didnt open!\n",argv[1]);
		exit(1);
	}

	keys=fopen("keys.txt","r");
	if(keys==NULL){
		printf("keys.txt file didnt open!\n");
		exit(1);
	}


	//fetch the public key
        getline(&e_str,&len,keys);
        e_str=strtok(e_str," ");
        for(int i=0;i<2;i++){
                e_str=strtok(NULL," ");
        }
        mpz_set_str(e,e_str,0);

        //fetch the modular
        getline(&N_str,&len,keys);
        N_str=strtok(N_str," ");
        for(int i=0;i<2;i++){
                N_str=strtok(NULL," ");
        }
        mpz_set_str(N,N_str,0);

        //fetch the private key
        getline(&d_str,&len,keys);
        d_str=strtok(d_str," ");
        for(int i=0;i<2;i++){
                d_str=strtok(NULL," ");
        }
        mpz_set_str(d,d_str,0);
        len=0; 
	//fetch p
        getline(&p_str,&len,keys);
        p_str=strtok(p_str," ");
        for(int i=0;i<2;i++){
                p_str=strtok(NULL," ");
        }
        mpz_set_str(p,p_str,0);
	len=0;
	//fetch q
        getline(&q_str,&len,keys);
        q_str=strtok(q_str," ");
        for(int i=0;i<2;i++){
                q_str=strtok(NULL," ");
        }
        mpz_set_str(q,q_str,0);
        len=0;
	//fetch Ip
        getline(&Ip_str,&len,keys);
        Ip_str=strtok(Ip_str," ");
        for(int i=0;i<2;i++){
                Ip_str=strtok(NULL," ");
        }
        mpz_set_str(Ip,Ip_str,0);
        fclose(keys);

	//gmp_printf("%Zu\n\n %Zu\n\n %Zu\n\n %Zu\n\n %Zu\n\n %Zu\n",e,d,N,p,q,Ip);

	//encrypt the document
	encrypt(plainData,e,d,N,p,q,Ip);
			
	//decrypt the document	
	decrypt(e,d,N,p,q,Ip);

	mpz_clears(e,d,N,p,q,Ip,NULL);
}
